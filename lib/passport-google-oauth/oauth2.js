...
Strategy.prototype.userProfile = function(accessToken, done) {
  this._oauth2.get('https://www.googleapis.com/plus/v1/people/me', accessToken, function (err, body, res) {
    ...
    try {
      ...
      if (json.emails) profile.emails = json.emails;

      if (json.email_verified || json.verified) profile.email_verified = json.email_verified ? json.email_verified : json.verified;
      if (json.hd) profile.domain = json.hd;
      ...
      if (json.link || json.url) { profile.profileUrl = json.link ? json.link : json.url; };
